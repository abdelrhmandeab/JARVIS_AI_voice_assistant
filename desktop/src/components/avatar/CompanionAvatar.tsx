...
// Jarvis brand blue (the "GYRO"-style wordmark blue) for the double-L logo.
const JARVIS_LOGO_BLUE = '#1F5BA6';
...
export function CompanionAvatar({ state, color }: AvatarProps) {
  ...
  // Frame + core glow use the shared state palette for every state (idle
  // included) so all avatars glow the same color.
  const active = color;
  // The Jarvis brand mark stays a fixed blue in every state.
  const logoColor = JARVIS_LOGO_BLUE;
  ...
  // Frame glow: the outer drop shadow + state-colored halo around the tile border.
  const frameGlow = [
  ...
  // Liquid-glass background lens box-shadow — layered inset highlights that read
  // as light refracted through a physical glass surface. Static design only:
  // no hover/press motion is ported over from the button version this came from.
  const lensBoxShadow = [
  ...
  return (
    ...
    >
      {/* Refraction filter for the liquid-glass background lens below. Chrome/Edge
          use it to physically distort the backdrop; Safari has no support for an
          SVG filter reference inside backdrop-filter and silently falls back to blur. */}
      <svg className="absolute h-0 w-0 overflow-hidden" aria-hidden="true">
        <filter id={`companion-liquid-glass-${filterId}`} primitiveUnits="objectBoundingBox">
          <feImage
            result="map"
            width="100%"
            height="100%"
      ...
      <div
        ...
        style={{
          position: 'relative',
          // Normalized to a ~150px visible body to match the other avatars.
          width: 150,
          height: 150,
          borderRadius: 21,
