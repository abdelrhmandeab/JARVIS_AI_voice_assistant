...
from core.config import (
    ...
    MEMORY_MAX_TURNS,
    MEMORY_PENDING_TASK_TTL_SECONDS,
...
_DEFAULT_PENDING_TASK_TTL_SECONDS = MEMORY_PENDING_TASK_TTL_SECONDS
_MAX_CLARIFICATION_PREFERENCES = 80
_MAX_APP_USAGE_ITEMS = 64
_MAX_COMMAND_USAGE_ROWS = MEMORY_COMMAND_USAGE_MAX_ROWS

# Phase 6 — which arg on a ParsedCommand is the "target" worth tracking per
# intent, for habit-aware resolution of vague commands.
_COMMAND_USAGE_TARGET_SLOT = {
...
# Canonicalizes preference key phrasing so "browser", "default browser", and
# "المتصفح بتاعي" all resolve to the same stored key.
...
_PREFERENCE_KEY_FILLER_PREFIXES = ("default ", "the ", "my ")


def _normalize_preference_key(key):
    value = _normalize_memory_token(key)
    if not value:
        return ""
    if value in _PREFERENCE_KEY_ALIASES:
        return _PREFERENCE_KEY_ALIASES[value]
    for prefix in _PREFERENCE_KEY_FILLER_PREFIXES:
        if value.startswith(prefix):
            value = value[len(prefix):].strip()
            break
    return _PREFERENCE_KEY_ALIASES.get(value, value)
...
class SessionMemory:
    ...
    def _default_context_slots(self):
        return {
            ...
            "pending_task": {},
            "pending_task_updated_at": 0.0,
            "user_preferences": {},
            "user_preferences_updated_at": 0.0,
            "command_usage": {},
            "command_usage_updated_at": 0.0,
        }
    ...
    def _load_context_slots(self, slots):
        ...
        command_usage_payload = slots.get("command_usage")
        payload["command_usage"] = dict(command_usage_payload) if isinstance(command_usage_payload, dict) else {}
        payload["command_usage_updated_at"] = self._as_timestamp(slots.get("command_usage_updated_at"))
        ...
        # Pending tasks must not survive an app restart (stale multi-turn
        # intent) — always start clean, same rationale as pending_clarification.
        payload["pending_task"] = {}
        payload["pending_task_updated_at"] = 0.0
        prefs_payload = slots.get("user_preferences")
        payload["user_preferences"] = dict(prefs_payload) if isinstance(prefs_payload, dict) else {}
        payload["user_preferences_updated_at"] = self._as_timestamp(slots.get("user_preferences_updated_at"))
        return payload
    ...
    def set_pending_task(self, payload, ttl_seconds=_DEFAULT_PENDING_TASK_TTL_SECONDS):
        """Phase 4 — store a multi-turn task (e.g. email/calendar/reminder) awaiting
        more slots. Unlike pending_clarification (one slot, one intent), a pending
        task carries an arbitrary partially-filled args dict across turns.
        """
        task = dict(payload or {})
        now_ts = time.time()
        task["created_at"] = self._as_timestamp(task.get("created_at")) or now_ts
        task["updated_at"] = now_ts
        task["ttl_seconds"] = max(5, int(ttl_seconds or _DEFAULT_PENDING_TASK_TTL_SECONDS))
        with self._lock:
            self._context_slots["pending_task"] = task
            self._context_slots["pending_task_updated_at"] = now_ts
            self._save()
        return True, "Pending task saved."

    def get_pending_task(self):
        """Return the pending task dict, or None if absent/expired (auto-clears)."""
        with self._lock:
            task = dict(self._context_slots.get("pending_task") or {})
            if not task:
                return None
            updated_at = self._as_timestamp(task.get("updated_at"))
            ttl_seconds = max(5, int(task.get("ttl_seconds") or _DEFAULT_PENDING_TASK_TTL_SECONDS))
            if updated_at and (time.time() - updated_at) > ttl_seconds:
                self._context_slots["pending_task"] = {}
                self._context_slots["pending_task_updated_at"] = 0.0
                self._save()
                return None
            return task
    ...
    def clear_pending_task(self):
        with self._lock:
            ...
            self._context_slots["pending_task_updated_at"] = 0.0
            self._save()
        return True, "Pending task cleared."
    ...
    def record_command_usage(self, parsed):
        """Phase 6 — track successful commands so vague ones can resolve by habit.

        Reuses the same normalize + cap pattern as ``record_app_usage``, but
        keyed by (intent, target-slot) so it generalizes beyond apps (e.g. a
        vague "open browser" resolving to the app most often opened for
        OS_APP_OPEN). Cheap RAM dict lookup — no vector/LLM involved.
        """
        intent = str(getattr(parsed, "intent", "") or "").strip().upper()
        args = dict(getattr(parsed, "args", None) or {})
        slot_name = _COMMAND_USAGE_TARGET_SLOT.get(intent)
        if not intent or not slot_name:
            return False, "command_usage not updated."
        raw_value = args.get(slot_name)
        value_key = _normalize_memory_token(raw_value)
        if not value_key:
            return False, "command_usage not updated."

        usage_key = f"{intent}::{slot_name}"
        now_ts = time.time()
        with self._lock:
            usage = dict(self._context_slots.get("command_usage") or {})
            bucket = dict(usage.get(usage_key) or {})
            row = dict(bucket.get(value_key) or {})
            row["count"] = int(row.get("count") or 0) + 1
            row["last_used_at"] = now_ts
            row["value"] = str(raw_value).strip()
            bucket[value_key] = row
            if len(bucket) > _MAX_COMMAND_USAGE_ROWS:
                sorted_items = sorted(
                    bucket.items(),
                    key=lambda item: float((item[1] or {}).get("last_used_at") or 0.0),
                    reverse=True,
                )
                bucket = dict(sorted_items[:_MAX_COMMAND_USAGE_ROWS])
            usage[usage_key] = bucket
            self._context_slots["command_usage"] = usage
            self._context_slots["command_usage_updated_at"] = now_ts
            self._save()
        return True, f"command_usage updated: {usage_key}={value_key}"

    def get_habitual_value(self, intent, slot_name=None):
        """Return the most-used value for (intent, slot) by count, or "" if none."""
        intent_key = str(intent or "").strip().upper()
        slot_key = slot_name or _COMMAND_USAGE_TARGET_SLOT.get(intent_key)
        if not intent_key or not slot_key:
            return ""
        usage_key = f"{intent_key}::{slot_key}"
        with self._lock:
            usage = dict(self._context_slots.get("command_usage") or {})
            bucket = dict(usage.get(usage_key) or {})
        if not bucket:
            return ""
        best_row = max(bucket.values(), key=lambda row: int((row or {}).get("count") or 0))
        return str(best_row.get("value") or "")

    def clear_command_usage(self):
        with self._lock:
            self._context_slots["command_usage"] = {}
            self._context_slots["command_usage_updated_at"] = 0.0
            self._save()
        return True, "command usage cleared."

    def set_user_preference(self, key, value, confidence=1.0, source="explicit"):
        """Phase 5 — persist a long-term user preference (e.g. default browser).

        Unlike pending_task/pending_clarification, preferences survive restarts.
        """
        pref_key = _normalize_preference_key(key)
        if not pref_key:
            return False, "preference not updated."
        now_ts = time.time()
        with self._lock:
            preferences = dict(self._context_slots.get("user_preferences") or {})
            preferences[pref_key] = {
                "value": str(value or "").strip(),
                "confidence": max(0.0, min(1.0, float(confidence or 1.0))),
                "source": str(source or "explicit").strip().lower(),
                "updated_at": now_ts,
            }
            self._context_slots["user_preferences"] = preferences
            self._context_slots["user_preferences_updated_at"] = now_ts
            self._save()
        return True, f"preference saved: {pref_key}={value}"

    def get_user_preferences(self):
        """Return {key: value} for all stored preferences (flattened, no metadata)."""
        with self._lock:
            preferences = dict(self._context_slots.get("user_preferences") or {})
        return {key: str(row.get("value") or "") for key, row in preferences.items()}
    ...
    def get_user_preference(self, key):
        ...
        with self._lock:
            preferences = dict(self._context_slots.get("user_preferences") or {})
            row = dict(preferences.get(pref_key) or {})
        return str(row.get("value") or "")

    def forget_user_preference(self, key):
        pref_key = _normalize_preference_key(key)
        if not pref_key:
        ...
        with self._lock:
            ...
            if pref_key not in preferences:
                return False, f"no preference set for: {pref_key}"
            del preferences[pref_key]
            self._context_slots["user_preferences"] = preferences
            self._context_slots["user_preferences_updated_at"] = time.time()
            self._save()
        return True, f"preference forgotten: {pref_key}"
