...
def handle(parsed):
    ...
    if action == "set_preference":
        key = parsed.args.get("key", "")
        value = parsed.args.get("value", "")
        ok, message = session_memory.set_user_preference(key, value, source="explicit")
        log_action(
        ...
        return True, f"Got it — {key}: {value}.", {}

    if action == "forget_preference":
        key = parsed.args.get("key", "")
        ok, message = session_memory.forget_user_preference(key)
        log_action(
            "memory_preference_forget",
            "success" if ok else "failed",
            details={"key": key},
        )
        if not ok:
            return False, message, {}
        return True, f"Forgot your {key} preference.", {}

    if action == "show_preferences":
        preferences = session_memory.get_user_preferences()
        if not preferences:
            return True, "You don't have any saved preferences yet.", {}
        lines = [f"{key}: {value}" for key, value in preferences.items()]
        return True, "Your preferences:\n" + "\n".join(lines), {}
